...
public class PoseEstimationWeightedAverage extends CommandBase {
    ...
    public PoseEstimationWeightedAverage(Limelight lime, Drivetrain drive) {
        ...
        poseEstimator = new DifferentialDrivePoseEstimator(Constants.Drivetrain.kDriveKinematics,
                drive.getRotation2d(), drive.getLeftEncoderDistance(), drive.getRightEncoderDistance(),
                drive.getPose());
    ...
    public void execute() {
        ...
        double apriltagPoseAverageRoll = filter
                .calculate(lime.getApriltagDimensionsFromFidicualId().getRotation().getX()
                        + lime.getAprilTagPoseRelativeToLimelight().getRotation().getX());
        double apriltagPoseAveragePitch = filter
                .calculate(lime.getApriltagDimensionsFromFidicualId().getRotation().getY() +
                        lime.getAprilTagPoseRelativeToLimelight().getRotation().getY());
        double apriltagPoseAverageYaw = filter
                .calculate(lime.getApriltagDimensionsFromFidicualId().getRotation().getZ() +
        ...
        double weightedAverageYaw = filter.calculate(
                apriltagPoseAverageYaw - lime.getPose3d().getRotation().getZ()
